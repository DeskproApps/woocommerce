...
import { CreateNote } from "../../../src/pages/Create/CreateNote";
...
describe("Create Note", () => {
  test("Creating a note should work correctly", async () => {
    ...
    await waitFor(() => {
      expect(Api.createOrderNote).toHaveBeenCalledTimes(1);
    });
  });
  ...
  test("Creating a note with no content should fail", async () => {
    ...
    await waitFor(() => {
      expect(Api.createOrderNote).toHaveBeenCalledTimes(0);
    });
  });
